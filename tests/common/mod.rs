...
// Generate notes directly in vault_root.
pub fn generate_test_vault(vault_root: &Path, note_count: usize) -> Result<(), Box<dyn Error>> {
    ...
    for i in 1..=note_count {
        titles.push(format!("Test Note {}", i));
    }
    let mut rng = rand::thread_rng();
    for i in 0..note_count {
        let title = &titles[i];
        let file_path = vault_root.join(format!("note_{}.md", i + 1));
        ...
        if add_links {
            let mut other_titles: Vec<&String> = titles.iter().filter(|t| *t != title).collect();
            other_titles.shuffle(&mut rng);
            let link_count = rng.gen_range(1..=3);
            let links: Vec<String> = other_titles
                .into_iter()
                .take(link_count)
                .map(|t| format!("[[{}]]", t))
                .collect();
